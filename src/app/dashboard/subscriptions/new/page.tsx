...
export default function NewSubscriptionPage() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        // Apenas prestadores (USER) e não ADMINS, a menos que os admins também paguem
        setUsersList(data.filter(u => u.role === "USER"));
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createSubscriptionPayment(formData);
  ...
  return (
    <PinGate>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-50 tracking-tight">Nova Mensalidade</h1>
          <p className="text-sm text-slate-400">Lance uma cobrança ou registre um pagamento de mensalidade.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            <div>
              <label htmlFor="userId" className="block text-sm font-medium text-slate-300 mb-1 flex items-center">
                <Users className="h-4 w-4 mr-2 text-slate-500" />
                Prestador (Usuário) *
              </label>
              {fetchingUsers ? (
                <div className="text-sm text-slate-400 py-2">Carregando usuários...</div>
              ) : (
                <select
                  name="userId"
                  id="userId"
